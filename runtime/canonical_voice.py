...
class VoiceBridge:
    ...
    async def _build_stt(self) -> STTAdapter:
        """Build a local STT adapter for microphone audio."""
        from speech.stt.local_factory import try_build_local_stt

        built = try_build_local_stt(
            repo_root=Path.cwd(),
        ...
        if not built.ready or built.provider is None:
            raise RuntimeError(f"local STT unavailable: {built.message}")
    ...
    async def _build_tts(self) -> TTSAdapter:
        """Build a local TTS adapter for assistant speech."""
        from speech.tts.local_factory import try_build_local_tts

        built = try_build_local_tts(
            repo_root=Path.cwd(),
            voice=self.config.tts_voice or "ru_RU-dmitri-medium",
            speed=self.config.tts_speed,
            volume=self.config.tts_volume,
            validate=True,
        )
        if not built.ready or built.provider is None:
            raise RuntimeError(f"local TTS unavailable: {built.message}")
        provider = built.provider
