    ...
    settings = validate_settings(
        {
            ...
            "voice_mic_device": "Built-in Microphone",
            "voice_speaker_device": "MacBook Speakers",
        }
    )

    assert settings.voice_stt_engine == "whisper"
    assert settings.voice_tts_engine == "edge_tts"
    assert settings.voice_mic_device == "Built-in Microphone"
    assert settings.voice_speaker_device == "MacBook Speakers"
    assert validate_settings(settings.to_dict()) == settings
...
@pytest.mark.parametrize(
    "payload,match",
    [
        ({"voice_stt_engine": "vosk"}, "voice_stt_engine"),
        ({"voice_tts_engine": "say"}, "voice_tts_engine"),
        ({"voice_mic_device": ""}, "voice_mic_device"),
        ({"voice_speaker_device": 1}, "voice_speaker_device"),
    ],
)
def test_rejects_invalid_voice_settings(payload, match):
