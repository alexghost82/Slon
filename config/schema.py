...
VOICE_STT_ENGINES = frozenset({"faster_whisper", "whisper", "no"})
VOICE_TTS_ENGINES = frozenset({"piper", "edge_tts", "no"})
...
def validate_settings(data: object) -> Settings:
    ...
    voice_stt_engine = _optional_enum(
        data, "voice_stt_engine", VOICE_STT_ENGINES, DEFAULT_VOICE_STT_ENGINE
    )
    voice_tts_engine = _optional_enum(
        data, "voice_tts_engine", VOICE_TTS_ENGINES, DEFAULT_VOICE_TTS_ENGINE
    )
    voice_mic_device = _optional_nullable_non_empty_str(data, "voice_mic_device")
    voice_speaker_device = _optional_nullable_non_empty_str(data, "voice_speaker_device")
...
def _optional_nullable_non_empty_str(
    data: Mapping[str, Any], field_name: str
) -> str | None:
    if field_name not in data:
        return None
